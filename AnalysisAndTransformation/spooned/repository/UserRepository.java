...
public class UserRepository implements IUserRepository {
    ...
    public boolean createUser(User user) throws UserCollectionException {
        Optional<User> userWrapper = this.users.stream().filter(u -> u.getEmail().equals(user.getEmail())).findFirst();
    ...
    public User connexion(String email, String password) throws UserCollectionException {
        Optional<User> userWrapper = this.users.stream().filter(user -> user.getEmail().equals(email) && user.getPassword().equals(password)).findFirst();
